...
public interface LoadBalance extends org.apache.dubbo.rpc.cluster.LoadBalance {
    ...
    class CompatibleLoadBalance implements com.alibaba.dubbo.rpc.cluster.LoadBalance {

        private final org.apache.dubbo.rpc.cluster.LoadBalance loadBalance;

        public CompatibleLoadBalance(org.apache.dubbo.rpc.cluster.LoadBalance loadBalance) {
            this.loadBalance = loadBalance;
        }

        @Override
        public <T> com.alibaba.dubbo.rpc.Invoker<T> select(List<com.alibaba.dubbo.rpc.Invoker<T>> invokers, com.alibaba.dubbo.common.URL url, com.alibaba.dubbo.rpc.Invocation invocation) throws RpcException {
            return new com.alibaba.dubbo.rpc.Invoker.CompatibleInvoker(this.loadBalance.select(invokers.stream().map(tInvoker -> tInvoker.getOriginal()).collect(Collectors.toList()), url, invocation));
        }
    }
}
